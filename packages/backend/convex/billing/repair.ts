...
async function syncBillingForUserId(
  ctx: ActionCtx,
  userId: Id<"user">
  ...
  const currentPeriodEnd = coerceTimestampMs(sub.periodEnd);
  const periodStart = coerceTimestampMs(sub.periodStart);
  ...
  const resolved = await ctx.runQuery(internal.billing.resolver.resolveActing, {
    userId,
  });
...
export const syncBillingForUser = internalAction({
  args: { userId: v.id("user") },
  handler: async (ctx, args) => syncBillingForUserId(ctx, args.userId),
});
export const resyncMySubscription = action({
  ...
  handler: async (ctx): Promise<ResyncResult> => {
    ...
    return syncBillingForUserId(ctx, identity.userId as Id<"user">);
  },
});
